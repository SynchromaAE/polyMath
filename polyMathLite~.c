 ...
 * 20 30 30 30 20 20   |      |    |    |    |      |      |
 * 16ths               |        |        |        |        |
 * ...easy to work out (7.5th or 2x15th) but the ability to command "split two 5ths into 3" is...just bump the sequence down one, change one entry and create a new entry after it (into the "duplication")
 */
...
static t_class *polyMathLite_tilde_class;
...
typedef struct _polyMathLite_tilde
{
  t_object x_obj;
  t_float f;
  t_groups grp;
  ...
  //writeGroup
  t_float Wsize, WsizeRem, WESize, groupOffset, Woff, WSInv;
  ...
  t_float PGcyc;
  //getVariables
  t_float clockOut, E_Acc1, E_Acc2, E_Acc3, E_Acc4, Pacc1, Pacc2, Pacc3, Pacc4, Pthis, PJoin;
  int Gnm;
  ...
  int o, p, q, r, s, t, u;
  int scramMeth, seqLen, halfSeq, swapsNum, doSwaps, swapNdx1, swapNdx2, swapFlag, iFSwapsNum, offsetVar, grpOffset;
...
} t_polyMathLite_tilde;

int rounder(t_polyMathLite_tilde *x, t_float f, int limit) // limiting round function
{
  x->fRound = f;
  x->iRound = (int)f;
...
static void getVariables(t_polyMathLite_tilde *x)
{
...
static void getVariations(t_polyMathLite_tilde *x)
{
...
void polyMathLite_tilde_pageTurn(t_polyMathLite_tilde *x)
{
  outlet_float(x->page, (t_float)x->pageNum);
}

void polyMathLite_tilde_eChange(t_polyMathLite_tilde *x)
{
  int varSlot;
...
void polyMathLite_tilde_preOut(t_polyMathLite_tilde *x, t_floatarg f)
{
  x->percentVal = f < 1 ? 0.1 : f > 99 ? 0.99 : f * 0.01;
}

void polyMathLite_tilde_altEarly(t_polyMathLite_tilde *x, t_floatarg f)
{
  x->altEarly = f > 0 ? 1 : 0;
}

void polyMathLite_tilde_cout(t_polyMathLite_tilde *x)
{
...
int writeGroup(t_polyMathLite_tilde *x, int group)
{
  //trying in setGroups instead, to see if it sticks
  /*  x->Wsize = x->Gn / x->Gd;
	  ...
	  for(x->d = 0; x->d < (int)x->Gn; x->d++)
	    {
	      ...
	      x->Woff = x->groupOffset + (x->WESize * (t_float)x->d);
	      x->WSInv = 1 / x->Wsize;
	      SETFLOAT(&x->seq.eSize[x->slot * MAXSEQ + x->Gstart + x->d], x->WESize);
	      SETFLOAT(&x->seq.eJoin[x->slot * MAXSEQ + x->Gstart + x->d], 1); // joins are set separately
	      SETFLOAT(&x->seq.jSize[x->slot * MAXSEQ + x->Gstart + x->d], x->WESize);
	      SETFLOAT(&x->seq.eSizeInv[x->slot * MAXSEQ + x->Gstart + x->d], 1 / x->WESize);
	      SETFLOAT(&x->seq.eOff[x->slot * MAXSEQ + x->Gstart + x->d], x->Woff);
	      SETFLOAT(&x->seq.allStep[x->slot * MAXSEQ + x->Gstart + x->d], (t_float)x->d + x->Gstart);
	      SETFLOAT(&x->seq.groupStep[x->slot * MAXSEQ + x->Gstart + x->d], (t_float)x->d);
	      SETFLOAT(&x->seq.groupNum[x->slot * MAXSEQ + x->Gstart + x->d], (t_float)group);
	      SETFLOAT(&x->seq.denom[x->slot * MAXSEQ + x->Gstart + x->d], x->getD);
	      if(x->myBug == 1) post("Step = %d, GStep = %d, WESize = %f, Woff = %f, Write: %d",x->seq.len[x->slot] + x->d, x->d, x->WESize, x->Woff, x->slot * MAXSEQ + x->Gstart + x->d);
	    }
	  x->seq.len[x->slot] = (int)x->Gn + (int)x->Gstart;
...
int reWriteSeq(t_polyMathLite_tilde *x)
{
...
void polyMathLite_tilde_addGroup(t_polyMathLite_tilde *x, t_symbol *s, int argc, t_atom *argv)
{
 if(argc == 3)
   {
	 ...
	 x->c = x->fGroup;
       ...
       {
	 x->c = x->maxGrp++; // do we need to store this variable in an array?
	                     // it strikes me that you can only add a group after a setGroups routine!
       }
     x->groupOffset = atom_getfloatarg(x->slot * GROUPS + x->maxGrp - 1, x->GROUPSIZE, x->grp.offset);
     if(x->Gn <= 0 || x->Gd <= 0)
       {
...
void polyMathLite_tilde_autoThreshold(t_polyMathLite_tilde *x, t_floatarg f)
{
  x->autoThreshold = f != 0 ? 1 : 0;
}

void polyMathLite_tilde_sizeThreshold(t_polyMathLite_tilde *x, t_floatarg f)
{
  x->sizeThreshold = f > 0.000001 ? f : 0.000001;
}

void polyMathLite_tilde_sizeFrac(t_polyMathLite_tilde *x, t_floatarg f)
{
...
int swapEventList(t_polyMathLite_tilde *x, int location, int slot, int var)
{
  x->isSwapList = 0;
  int varSeq = var - 1;
  if(var > 0)
    {
      ...
      SETFLOAT(&x->eventList[12], atom_getfloatarg(slot * MAXSEQ + varSeq * x->SEQSIZE + location, x->VARSIZE, x->var.pAcc1));
      SETFLOAT(&x->eventList[13], atom_getfloatarg(slot * MAXSEQ + varSeq * x->SEQSIZE + location, x->VARSIZE, x->var.pAcc2));
      SETFLOAT(&x->eventList[14], atom_getfloatarg(slot * MAXSEQ + varSeq * x->SEQSIZE + location, x->VARSIZE, x->var.pAcc3));
      SETFLOAT(&x->eventList[15], atom_getfloatarg(slot * MAXSEQ + varSeq * x->SEQSIZE + location, x->VARSIZE, x->var.pAcc4));
      SETFLOAT(&x->eventList[16], atom_getfloatarg(slot * MAXSEQ + varSeq * x->SEQSIZE + location, x->VARSIZE, x->var.eSizeInv));
      SETFLOAT(&x->eventList[17], atom_getfloatarg(slot * MAXSEQ + varSeq * x->SEQSIZE + location, x->VARSIZE, x->var.denom));
      SETFLOAT(&x->eventList[18], atom_getfloatarg(slot * MAXSEQ + varSeq * x->SEQSIZE + location, x->VARSIZE, x->var.altOff));
      x->isSwapList = 1;
    }
  else
    {
      ...
      SETFLOAT(&x->eventList[12], atom_getfloatarg(slot * MAXSEQ + location, x->SEQSIZE, x->seq.pAcc1));
      SETFLOAT(&x->eventList[13], atom_getfloatarg(slot * MAXSEQ + location, x->SEQSIZE, x->seq.pAcc2));
      SETFLOAT(&x->eventList[14], atom_getfloatarg(slot * MAXSEQ + location, x->SEQSIZE, x->seq.pAcc3));
      SETFLOAT(&x->eventList[15], atom_getfloatarg(slot * MAXSEQ + location, x->SEQSIZE, x->seq.pAcc4));
      SETFLOAT(&x->eventList[16], atom_getfloatarg(slot * MAXSEQ + location, x->SEQSIZE, x->seq.eSizeInv));
      SETFLOAT(&x->eventList[17], atom_getfloatarg(slot * MAXSEQ + location, x->SEQSIZE, x->seq.denom));
      SETFLOAT(&x->eventList[18], atom_getfloatarg(slot * MAXSEQ + location, x->SEQSIZE, x->seq.altOff));
      x->isSwapList = 1;
    }
  return(x->isSwapList);
}

int addEventList(t_polyMathLite_tilde *x, int location, int len, int slot, int var)
{
  int isAdded = 0;
  int varSeq = var - 1;
  if(var > 0)
    {
      ...
      SETFLOAT(&x->var.pAcc1[slot * MAXSEQ + varSeq * x->SEQSIZE + location], atom_getfloatarg(12, EVENTLIST, x->eventList));
      SETFLOAT(&x->var.pAcc2[slot * MAXSEQ + varSeq * x->SEQSIZE + location], atom_getfloatarg(13, EVENTLIST, x->eventList));
      SETFLOAT(&x->var.pAcc3[slot * MAXSEQ + varSeq * x->SEQSIZE + location], atom_getfloatarg(14, EVENTLIST, x->eventList));
      SETFLOAT(&x->var.pAcc4[slot * MAXSEQ + varSeq * x->SEQSIZE + location], atom_getfloatarg(15, EVENTLIST, x->eventList));
      SETFLOAT(&x->var.eSizeInv[slot * MAXSEQ + varSeq * x->SEQSIZE + location], atom_getfloatarg(16, EVENTLIST, x->eventList));
      SETFLOAT(&x->var.denom[slot * MAXSEQ + varSeq * x->SEQSIZE + location], atom_getfloatarg(17, EVENTLIST, x->eventList));
      SETFLOAT(&x->var.altOff[slot * MAXSEQ + varSeq * x->SEQSIZE + location], atom_getfloatarg(18, EVENTLIST, x->eventList));
      isAdded = 1;
    }
  else
    {
      ...
      SETFLOAT(&x->seq.pAcc1[slot * MAXSEQ + location], atom_getfloatarg(12, EVENTLIST, x->eventList));
      SETFLOAT(&x->seq.pAcc2[slot * MAXSEQ + location], atom_getfloatarg(13, EVENTLIST, x->eventList));
      SETFLOAT(&x->seq.pAcc3[slot * MAXSEQ + location], atom_getfloatarg(14, EVENTLIST, x->eventList));
      SETFLOAT(&x->seq.pAcc4[slot * MAXSEQ + location], atom_getfloatarg(15, EVENTLIST, x->eventList));
      SETFLOAT(&x->seq.eSizeInv[slot * MAXSEQ + location], atom_getfloatarg(16, EVENTLIST, x->eventList));
      SETFLOAT(&x->seq.denom[slot * MAXSEQ + location], atom_getfloatarg(17, EVENTLIST, x->eventList));
      SETFLOAT(&x->seq.altOff[slot * MAXSEQ + location], atom_getfloatarg(18, EVENTLIST, x->eventList));
      isAdded = 1;
    }
  return(isAdded);
}

int oneToTheRightOrLeft(t_polyMathLite_tilde *x, int location, int len, int slot, int var, int dir)
...
int onePToTheLeftOrRight(t_polyMathLite_tilde *x, int location, int len, int slot, int var, int P, int direction)
{ //tests are carried out in the parent function to make sure this is a valid swap
  int swapShuffle = 0;
  int location2 = 0;
...
int oneEToTheLeftOrRight(t_polyMathLite_tilde *x, int location, int len, int slot, int var, int P, int direction)
{ //tests are carried out in the parent function to make sure this is a valid swap
  int swapShuffle = 0;
  int location2 = 0;
  if(direction == -1) location2 = location - 1;
...
void polyMathLite_tilde_swapElement(t_polyMathLite_tilde *x, t_symbol *s, int argc, t_atom *argv)
{
  int process = 0;
  int shuffled = 0;
  if(argc == 2)
...
void polyMathLite_tilde_initSeqSlot(t_polyMathLite_tilde *x, t_floatarg newSeqSlot, t_floatarg isSeq)
{
 ...
 */
void polyMathLite_tilde_seqInSlot(t_polyMathLite_tilde *x, t_symbol *s, int argc, t_atom *argv)
{
...
/*typedef struct _vars
{
  ...
  int swapsRef[MAXSEQ * 2];
  int swapped[MAXSEQ];*/
void polyMathLite_tilde_groupScramble(t_polyMathLite_tilde *x, t_symbol *s, int argc, t_atom *argv)
{
  //args: slot, var, destVar, scramRand, mode
  //if mode == 0, just do that percentage of scrambles
  //if mode == 1, ensure exactly that number of unique displacements is carried out
  
  
}

void polyMathLite_tilde_groupInSlot(t_polyMathLite_tilde *x, t_symbol *s, int argc, t_atom *argv)
{
...
void polyMathLite_tilde_thisSlot(t_polyMathLite_tilde *x, t_floatarg f)
{
  x->thisSlot = f < 0 ? 0 : f >= SLOTS ? SLOTS - 1 : (int)f;
}

void polyMathLite_tilde_groupThisSlot(t_polyMathLite_tilde *x, t_symbol *s, int argc, t_atom *argv)
{
...
void polyMathLite_tilde_setGroups(t_polyMathLite_tilde *x, t_symbol *s, int argc, t_atom *argv)
  ...
  if(argc >= 3)
	...
	//      for(x->c = 0; x->c < (argc - 1) / 2; x->c++)
	{
	  ...
	  if(x->Gn <= 0 || x->Gd <= 0)
	    ...
	    {
	      // trying this here instead of in writeGroup
	      x->Wsize = x->Gn / x->Gd;
	      if(x->myBug > 0) post("Wsize = %f",x->Wsize);
	      SETFLOAT(&x->grp.n[x->slot * GROUPS + x->c],x->Gn);
	      SETFLOAT(&x->grp.d[x->slot * GROUPS + x->c],x->Gd);
...
void polyMathLite_tilde_setP(t_polyMathLite_tilde *x, t_symbol *s, int argc, t_atom *argv)
{
...
void polyMathLite_tilde_setPOnly(t_polyMathLite_tilde *x, t_symbol *s, int argc, t_atom *argv)
{
...
void polyMathLite_tilde_setVOnly(t_polyMathLite_tilde *x, t_symbol *s, int argc, t_atom *argv)
{
...
void polyMathLite_tilde_slot(t_polyMathLite_tilde *x, t_floatarg f)
{
...
void polyMathLite_tilde_slotLen(t_polyMathLite_tilde *x, t_floatarg f)
{
...
//useless: lastLen
void polyMathLite_tilde_jumpNext(t_polyMathLite_tilde *x, t_symbol *s, int argc, t_atom *argv)
{
...
void polyMathLite_tilde_jumpTo(t_polyMathLite_tilde *x, t_symbol *s, int argc, t_atom *argv) // Jan 17th 2018
{
  //int lastOffset, nextOffset;
...
void polyMathLite_tilde_variation(t_polyMathLite_tilde *x, t_floatarg f) // OBSOLETE when starting from !0 - use _jumpTo instead
{
  //THIS WILL NEED TO BE REVISITED after the completion of the re-factoring of polyMathLite_tilde_nextSlot in the perf function!
...
int copySeq(t_polyMathLite_tilde *x, int slot, int varOffset)
{
  x->copyWell = 1;
...
int scrambleSeq(t_polyMathLite_tilde *x, int varOffset, int slot)
{
  x->scramWell = 1;
  for(x->q = 0; x->q < x->doSwaps; x->q++) 
    {
      //int scrambleSeq(t_polyMathLite_tilde *x, int slot, int varOffset, int var, int len, t_float prob)
...
int regroup(t_polyMathLite_tilde *x, int grpOffset, int varOffset, int slot, int var, int len)
{
  x->groupWell = 1;
  x->VGnm = 0;
  x->VGCount = 0;
...
int varOffsets(t_polyMathLite_tilde *x, int varOffset, int len)
{
  x->varWrite = 1;
  x->varOff = 0;
  x->swapVal = 0;
...
int scrambleSwaps(t_polyMathLite_tilde *x, int slot, int varOffset, int var, int len, t_float prob, int grpOffset)
{
  x->swapWell = 1;
...
void polyMathLite_tilde_scramble(t_polyMathLite_tilde *x, t_symbol *s, int argc, t_atom *argv)
{
  if(argc < 1 && argc > 3)
    {
...
void polyMathLite_tilde_scramMeth(t_polyMathLite_tilde *x, t_floatarg f)
{
  x->scramMeth = f != 0 ? 1 : 0;
}

void polyMathLite_tilde_eMult(t_polyMathLite_tilde *x, t_floatarg f)
{
  x->eMult = f > 0 ? 1 : 0;
}

void polyMathLite_tilde_preChange(t_polyMathLite_tilde *x, t_floatarg f) // note that this will need some alteration / intervention in the timeline when nextSlot is implemented in the perf function! - 12th Jan 2018
{
  x->percentVal = f <= 0.1 ? 0.1 : f > 99.9 ? 99.9 : f;
}

// JOINED ELEMENTS
void polyMathLite_tilde_makeJoin(t_polyMathLite_tilde *x, t_symbol *s, int argc, t_atom *argv)
{
  x->joinSuccess = 0;
...
void polyMathLite_tilde_joinSeq(t_polyMathLite_tilde *x, t_symbol *s, int argc, t_atom *argv)
{
  if(argc > 2)
    {
      x->JSlot = (int)atom_getfloat(argv);
...
void polyMathLite_tilde_initSlot(t_polyMathLite_tilde *x, t_floatarg f)
{
...
void polyMathLite_tilde_altOut(t_polyMathLite_tilde *x, t_floatarg f)
{
  x->altOut = f != 0 ? 1 : 0;
  if(f > 0 && f < 9)
    {
...
void polyMathLite_tilde_init(t_polyMathLite_tilde *x, t_symbol *s)
{
  // counters init:
...
void polyMathLite_tilde_getSeq(t_polyMathLite_tilde *x, t_symbol *s, int argc, t_atom *argv)
{
  if(argc == 3)
    {
...
// DEBUG CODE
void polyMathLite_tilde_debug(t_polyMathLite_tilde *x, t_floatarg myBug)
{
  int bug = (int)myBug;
...
// JOIN ROUTINES
static void checkJoinsOut(t_polyMathLite_tilde *x)
{
...
static void checkJoinsVarOut(t_polyMathLite_tilde *x)
{
  if(x->PJoined > 1)
    {
      x->PJoined--; // 3, 2, 1
...
void polyMathLite_tilde_setBpm(t_polyMathLite_tilde *x, t_floatarg f)
{
  if(f > 0)
    {
      x->BPM = f;
...
{
  t_polyMathLite_tilde *x = (t_polyMathLite_tilde *)(w[1]);
...
void polyMathLite_tilde_dsp(t_polyMathLite_tilde *x, t_signal **sp)
{
  dsp_add(polyMathLite_tilde_perform, 6, x, sp[0]->s_vec, sp[1]->s_vec, sp[2]->s_vec, sp[3]->s_vec, sp[0]->s_n);
}

static void *polyMathLite_tilde_new()
{
  t_polyMathLite_tilde *x = (t_polyMathLite_tilde *)pd_new(polyMathLite_tilde_class);
  ...
  // then re-write polyMathLite_tilde_init
  x->Gstep = 0;
  x->Gn = 16;
  x->Gd = 16;
  x->ESize = 0.0625;
  ...
  x->fOut = clock_new(x, (t_method)polyMathLite_tilde_cout);
  x->early = clock_new(x, (t_method)polyMathLite_tilde_eChange);
  x->pageTurner = clock_new(x, (t_method)polyMathLite_tilde_pageTurn);
  x->barNew = 0;
  getVariables(x);
  return (x);
}
...
{
  polyMathLite_tilde_class = class_new(gensym("polyMathLite~"), (t_newmethod)polyMathLite_tilde_new, 
			      0, sizeof(t_polyMathLite_tilde), 0, A_GIMME, 0);
    CLASS_MAINSIGNALIN(polyMathLite_tilde_class, t_polyMathLite_tilde, f);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_dsp, gensym("dsp"), 0);

    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_init, gensym("init"), A_DEFFLOAT, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_slot, gensym("slot"), A_DEFFLOAT, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_setGroups, gensym("setGroups"), A_GIMME, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_addGroup, gensym("addGroup"), A_GIMME, 0);

    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_groupInSlot, gensym("groupInSlot"), A_GIMME, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_groupThisSlot, gensym("groupThisSlot"), A_GIMME, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_thisSlot, gensym("thisSlot"), A_DEFFLOAT, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_jumpTo, gensym("jumpTo"), A_GIMME, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_jumpNext, gensym("jumpNext"), A_GIMME, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_autoThreshold, gensym("autoThreshold"), A_DEFFLOAT, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_sizeThreshold, gensym("sizeThreshold"), A_DEFFLOAT, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_sizeFrac, gensym("sizeFrac"), A_DEFFLOAT, 0);

    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_joinSeq, gensym("setJoins"), A_GIMME, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_makeJoin, gensym("makeJoin"), A_GIMME, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_setP, gensym("pSet"), A_GIMME, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_setPOnly, gensym("pSetOnly"), A_GIMME, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_setVOnly, gensym("vSetOnly"), A_GIMME, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_altOut, gensym("altOut"), A_DEFFLOAT, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_preOut, gensym("precent"), A_DEFFLOAT, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_eMult, gensym("eMult"), A_DEFFLOAT, 0);

    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_variation, gensym("variation"), A_DEFFLOAT, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_scramble, gensym("scramble"), A_GIMME, 0);

    class_addmethod(polyMathLite_tilde_class, (t_method)getVariables, gensym("getVariables"), A_DEFFLOAT, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_debug, gensym("debug"), A_DEFFLOAT, 0);

    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_swapElement, gensym("swap"), A_GIMME, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_seqInSlot, gensym("seqUnit"), A_GIMME, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_initSeqSlot, gensym("initSeqSlot"), A_DEFFLOAT, A_DEFFLOAT, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_groupScramble, gensym("scrambleGroups"), A_GIMME, 0);

    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_getSeq, gensym("getSequence"), A_GIMME, 0);
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_slotLen, gensym("slotLength"), A_GIMME, 0);
    
    class_addmethod(polyMathLite_tilde_class, (t_method)polyMathLite_tilde_setBpm, gensym("bpm"), A_DEFFLOAT, 0);
}
